...
bronze_path=f"gs://{GCS_BUCKET}/landing/retailer-db/order_items/*.json"
silver_path=f"gs://{GCS_BUCKET}/silver/order_items/"

print("▶️ Processing Table: order_items SCD2")
...
changed_records = (
    ...
    .join(
        ...
        (col("t.order_item_id") == col("s.order_item_id")) & (col("t.is_active") == True),
        "inner"
    )
    .filter(
        (
            (col("t.order_id") != col("s.order_id")) |
            (col("t.product_id") != col("s.product_id")) |
            (col("t.quantity") != col("s.quantity")) |
            (col("t.price") != col("s.price")) |            
...
silver_expired = (
    active_silver_df.alias("t")
    .join(changed_records, "order_item_id", "leftsemi")
...
print("▶️ Silver layer table order_items saved")